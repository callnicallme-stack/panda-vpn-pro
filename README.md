...
- 🗓️ 2026.02.27 🗓️ `农夫山泉` 的用户请注意，该机场目前正遭受攻击，官网无法访问，且所有节点都处于不可用的状态，存在一定跑路风险（看老板能不能承受住压力了）。此通知暂不删除。🗓️ 2026.02.27 🗓️ 目前官网时不时会恢复，但节点仍持续遭受攻击。🗓️ 2026.03.01 🗓️ 目前该机场的官网与节点已恢复。但是，为了防止攻击与通报，**已经改为强制使用客户端的方式了**（官网的“使用文档”中有客户端的下载说明），我已经给其添加 🔒 标识，正文上方也已新增该标识的具体说明。不过，这种强制客户端的方式肯定是有人不满意的，比如我自己就是不喜欢的。然而，在现在这种“外有通报，内有攻击”的情况下，我虽不支持，但也理解，没有被打跑路已经是不幸中的万幸（对机场圈关注较多的人应该知道，此次的攻击者这两年已经打跑了不少机场，尤其是小机场，因为防御成本是远高于攻击成本的，而小机场那点利润是上不了多强的防御的。大机场虽钱多能上防御，但其实也被折腾的够呛）。总之，如果实在介意的话，可在套餐用完后更换其他机场，又或者，尝试发工单要求退款，再或者，有钱任性直接不用了。话说回来，这种方式是否能有效防止攻击还有待继续观察，4.15 前若能保持基本稳定，此通知将删除。🗓️ 2026.03.24 🗓️ 目前再次遭受攻击，官网与客户端均无法使用。🗓️ 2026.03.24 🗓️ 已恢复（4.15 前如果持续遭受攻击，会延长该通知的保留时间，甚至会考虑暂推该机场）。🗓️ 2026.03.30 🗓️ 近期客户端已经连续更新了几个版本，旧版本由于存在较多漏洞，都已被弃用。如发现节点无法使用，请第一时间前往官网查看是否有新版本客户端（另外，当前最新版本不会覆盖旧版本），等趋于稳定之后应该就不会频繁更新了。🗓️ 2026.04.12 🗓️ 该机场的老板在前几日已将机场交由其他人运营，然而新老板似乎有点摆烂，因此决定暂不推荐该机场（观察一定时间后可能会重新推荐），已购用户是否续费请自行决定。此通知及该机场的链接将于 5.1 删除。
- 🗓️ 2026.03.31 🗓️ `自由猫` 的用户请注意，该机场表示自下个月末起，将不再支持订阅链接，只能使用该机场的客户端。不过，在此期间，订阅链接的可用性也不一定会保证。因此，如果愿意继续使用，建议尽快使用其客户端（不过，当前形势下，任何中转机场随时都可能变成强制客户端的方式，所以，如果实在无法接受这种方式，或许只能选择直连机场）。5.1 前若没有转机，此通知将删除，且会在机场列表为其添加 🔒 标识。
...
- 🗓️ 2026.04.03 🗓️ `蜂窝云` 的用户请注意，该机场目前已关闭订阅链接（此前还可通过发送工单获取），只能使用官方客户端，我已为其添加 🔒 标识。不过，之前已经获取过订阅链接的，也还能用（少部分人可能不行），至于能用多久就不知道了。后续如果能恢复手动获取，会另行通知。此通知将于 4.15 删除。🗓️ 2026.04.03 🗓️ 已恢复正常订阅，但是节点这两天会改成直连节点。我已将 🔒 标识移除，并将机场类型改为“直连”。之后能否恢复中转不得而知。🗓️ 2026.04.09 🗓️ 截至目前其中转节点仍然保留着，考虑到近期我是不推荐中转机场的，并且其直连节点当前阶段的体验在我看来，是不如本就为直连机场的节点的（虽然该机场本来也是有直连节点的，但是并没有被作为维护重心），因此已将其类型重新修改为“中转”。🗓️ 2026.04.13 🗓️ 目前该机场再次变为强制客户端，不过，这次没有完全关闭订阅链接。订阅链接依然可用，但订阅只能获取直连节点，若想使用中转节点，需使用其客户端。我已在机场列表重新为其添加 🔒 标识，并在机场列表下方新增了一些补充说明。此通知延至 5.1 删除。
- 🗓️ 2026.04.15 🗓️ `m78星云` 的用户请注意，该机已变为强制客户端，订阅链接虽依然可用，但订阅只能获取直连节点，若想使用中转节点，需使用其客户端。我已在机场列表为其添加 🔒 标识，并在机场列表下方新增了一些补充说明。此通知将于 5.1 删除。
...
| :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| 2019.08.15 | [壹速云](https://www.onesy1.cc/auth/register?code=R4Lm)<br>📜 | 中转 | 1 | 10.90 / 150 GB | 188.00 / 1000 GB | Null | Null |
| 2020.04.22 | [次元雲](https://ciyy.one/#/register?code=tsL8Me6h)<br>`A` | 中转 | 1 | 17.00 / 256 GB | 90.00 / 520 GB | Null | Null |
...
| 2023.02.03 | 🟨<br>[自由猫](https://us.freecat.cloud/register?code=8S3V1vsr) | 中转 | 1 | 9.00 / 100 GB | 50.00 / 500 GB | Null | 8 折<br>`FREECAT`<br>所有套餐可用<br>限用一次<br>长期有效 |
| 2023.04.15 | [鱼鱼](https://252288.xyz/#/register?invite_code=lw92ZJPe)<br>`*AX`<br>🔥🔥<br>📜 | 直连<br>+ | 1 | 7.99 / 128 GB | 20.00 / 100 GB | Null | Null |
| ~~2023.05.01~~ | 🟨<br>~~[农夫山泉](https://qqq.nfsqttt.com/#/register?code=HvoPMFli)~~<br>🔒<br>暂停推荐 | ~~中转~~ | ~~1~~ | ~~8.00 / 200 GB~~ | ~~15.00 / 200 GB~~ | ~~Null~~ | ~~Null~~ |
| 2023.07.01 | 🟨<br>[蜂窝云](https://api.fwcloud.life/auth/register?code=DZzGx5)<br>🔒<br>📜 | 中转 | 0.5 & 1 | 20.00 / 200 GB | Null | Null | 9 折<br>`FW9`<br>所有套餐可用<br>长期有效 |
| 2023.09.12 | 🟨<br>[M78星云](https://m78star.cloud/#/register?code=7IWr2dOP)<br>`A`<br>🔒<br>📜 | 中转 | 1 |  7.80 / 80 GB | 99.00 / 400 GB | Null | Null |
| 2024.03.01 | [兔兔云](https://www.tutuyun.uk/auth/register?code=QKEtXiLs) | 中转 | 1 | 11.88 / 140 GB | 18.00 / 60 GB | 12 hours | Null |
| 2024.07.12 | [Doriya](https://dash.rtx.al/#/register?code=hAlv337j) | 中转 | 1 | 8.00 / 100 GB | Null | Null | Null |  
| 2024.07.18 | [FlyBit](https://flybit.vip/#/register?code=HpHWTZX1) | 中转 | 1.5 | 15.00 / 128 GB | 36.00 / 128 GB | 2 GB | 9 折<br>`flybit`<br>所有套餐可用<br>长期有效 |
| 2024.08.14 | [NyanSS](https://billing.nyanss001.top/register?code=dfsKFF7s) | 中转 | 1 | 7.50 / 50 GB | 44.90 / 200 GB | Null | Null |
| 2025.03.07 | [ofoNET](https://ofotw.org/#/register?code=q3kx6Xt9) | 中转 | 1 | 12.87 / 200 GB | Null | Null | Null |
...
使用代理 / VPN 前，建议确认你的本地路由器不存在 Telnet / SSH 对外暴露或后门风险。若需进行无损的本地安全检查，可参阅：[路由器 Telnet / SSH 暴露安全检查指南](docs/router-security-check.md)。
